...
import com.zhou.zoj.model.dto.user.UserAddRequest;
import com.zhou.zoj.model.dto.user.UserLoginRequest;
import com.zhou.zoj.model.dto.user.UserQueryRequest;
import com.zhou.zoj.model.dto.user.UserRegisterRequest;
import com.zhou.zoj.model.dto.user.UserUpdateMyRequest;
import com.zhou.zoj.model.dto.user.UserUpdateRequest;
...
//    private static final String SALT = "yupi";
    ...
    public BaseResponse<Boolean> updateMyUser(@RequestBody UserUpdateMyRequest userUpdateMyRequest,
        ...
        String updatePassword = userUpdateMyRequest.getUpdatePassword();
        if (updatePassword != null) {
            if (updatePassword.length() < 8) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "密码设置过短，至少8位");
            }
            //说明需要更新密码
            String AccessPassword = userUpdateMyRequest.getAccessPassword();
            if (AccessPassword != null) {
                String encryptAccessPassword = DigestUtils.md5DigestAsHex((SALT + AccessPassword).getBytes());
                if (encryptAccessPassword.equals(userUpdateMyRequest.getUserPassword())) {
                    // 2. 加密
                    String encryptPassword = DigestUtils.md5DigestAsHex((SALT + updatePassword).getBytes());
                    userUpdateMyRequest.setUserPassword(encryptPassword);
                } else {
                    return ResultUtils.error(1, "原密码不正确");
                }
            }
        }
